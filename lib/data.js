...
if(CFG.redis && CFG.redis.host && CFG.redis.port){
    CFG.redis.pub = redis.createClient(CFG.redis.port, CFG.redis.host, CFG.redis.options);
    CFG.redis.sub = redis.createClient(CFG.redis.port, CFG.redis.host, CFG.redis.options);
    CFG.redis.channel = CFG.redis.channel || 'kaskade_redis';
    CFG.redis.sub.subscribe(CFG.redis.channel);
    
    CFG.redis.sub.on('message', function(channel, message){
        if(channel == CFG.redis.channel){
            var obj = JSON.parse(message);
            
            var collection = self.Collection.collections[obj.collection];
            var method = obj.method;
            var record = obj.record;
            
            collection.__eventEmitter.emit('mutation', method, record);
        }
    });
