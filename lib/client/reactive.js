...
self.Class = (function(){
    ...
    var Class = function(proto, init){
        ...
        _Class.observeKey = function(collection, key, done){
            var query = {};
            query[collection.keyField] = key;
            
            if(typeof done === 'function'){
                collection.observe(query, function(mutation, record){
                    if(mutation == 'init'){
                        done(_Class.observe(record[key]));
                    }
                });
            }else{
        ...
        _Class.prototype.$observe = function(record){
            var _class = this;
            for(var f in record){
                (function(f){
                    if( !(/^__.+__$/.test(f) || /^\$/.test(f)) ){
        ...
        _Class.prototype.$observeKey = function(collection, key, done){
...
self.List = (function(){
    ...
    List.prototype.init = function(items){
        ...
        for(var i=0, j=list.items.length; i<j; i++){
            list.items[i] = undefined;
        }
        list.items.length = 0;
    ...
    List.prototype.observe = function(collection, query){
        var list = this;
        collection.observe(query, function(method, record){
            switch(method){
                case 'init':
