...
var queryToSQL = function(query){
    ...
    for(var f in query){
        if(query.hasOwnProperty(f)){
            if(query[f] === null)
                str += f+' IS NULL ';
            else if(! (query[f] instanceof Array) )
                str += f+'="'+query[f]+'" ';
            else
...
self.SQLQueryParser = (function(){
    ...
    var SQLQueryParser = function(cfg){
        ...
        sqlQueryParser.preParser = cfg.preParser || function(conn, query, record, done){return done(null, query, record);};
    };
    ...
    SQLQueryParser.prototype.test = function(conn, query, record, done){
        ...
        sqlQueryParser.preParser(conn, query, record, function(err, query, record){
    ...
    SQLQueryParser.prototype.select = function(conn, query, done){
        ...
        sqlQueryParser.preParser(conn, query, null, function(err, query){
