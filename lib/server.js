...
module.exports.create = function create() {
    ...
    self.sock = self.sockjs.createServer({sockjs_url: CFG.host + '/kaskade.js'});
    self.sock.on('connection', function(conn) {
        ...
        conn.on('close', function() {
            ...
            for(var c in Collection.collections){
                delete Collection.collections[c].observers[conn.id];
            }
            delete self.clients[conn.id];
            CFG.onConnectionClose(conn);
        });
